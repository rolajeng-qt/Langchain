...
def recognize_speech():
    ...
    try:
        ...
        print("正在识别语音...(recognizing speech...)")
        # 可以根据用户环境选择语言
        text = recognizer.recognize_google(audio, language="zh-CN")
        print(f"识别结果: {text}")
        return text
    except sr.WaitTimeoutError:
        return "没有检测到语音输入 (No speech detected)"
    except sr.UnknownValueError:
        return "无法识别语音 (Could not understand audio)"
    except sr.RequestError as e:
        return f"语音识别服务错误: {e} (Speech recognition service error)"
    except Exception as e:
        return f"发生错误: {e} (An error occurred)"
...
def main():
    ...
    print("Type 'voice' or '語音' to use voice input (輸入 'voice' 或 '語音' 使用語音輸入)")
    print("Example: '開啟相機 等待5秒 拍一張照片 並存到C:\\Users\\username\\Documents\\photo.png'")
    
    while True:
        print("\nChoose input method (選擇輸入方式):")
        print("1. Text input (文字輸入)")
        print("2. Voice input (語音輸入)")
        input_choice = input("Enter your choice (輸入你的選擇) [1/2]: ")
        
        if input_choice == "2":
            print("Voice input mode activated (語音輸入模式已啟動)")
            user_input = recognize_speech()
            if "error" in user_input.lower() or "无法识别" in user_input or "没有检测到" in user_input:
                print(f"Speech recognition issue: {user_input}")
                continue
        else:
        ...
        try:
            ...
            if any(keyword in user_input for keyword in ["開啟相機", "等待", "拍一張照片", "存到", 
                                                       "打开相机", "启动相机", "等待", "拍照", "保存到",
                                                       "open camera", "start camera", "wait", "take photo", "save to"]):
